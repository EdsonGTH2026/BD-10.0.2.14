...
create procedure [dbo].[pConsultaMonitorINTLPF] (@codusuario varchar(20))
as
BEGIN
/*
--<<<<comentar
...
FinamigoSIC.dbo.vES as vES,
FinamigoSIC.dbo.vPN as vPN 
inner join FinamigoSIC.dbo.vINTL as INTL on INTL.codusuario = vpn.codusuario
left join FinamigoSIC.dbo.vPA as vPA_F on vPA_F.TipoDir = 'FAMILIAR' and vPA_F.codusuario = vpn.codusuario
left join FinamigoSIC.dbo.vPA as vPA_N on vPA_N.TipoDir = 'NEGOCIO' and vPA_N.codusuario = vpn.codusuario
inner join FinamigoSIC.dbo.vPI as vPI on vPI.codusuario = vPN.codusuario
where vPN.codusuario = @codusuario --'CGB901111F6415'
order by vPI.fechadesembolso 
