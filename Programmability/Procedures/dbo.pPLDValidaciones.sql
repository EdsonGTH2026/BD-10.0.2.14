...
GO
CREATE PROCEDURE [dbo].[pPLDValidaciones](@Codigo VARCHAR(25), @MontoDeposito MONEY, @CodSistema VARCHAR(3), 
										  @CodOficina VARCHAR(4), @Opcion VARCHAR(50), @Ret INT OUTPUT)
AS
SET NOCOUNT ON;
BEGIN
	/*GDF-20220622
	...
	*/
	...
	BEGIN
		EXEC pSDValidaLimiteDepositosEfectivo @Codigo, @MontoDeposito, @CodSistema, @CodOficina, @Ret OUTPUT
		...
		IF UPPER(@Opcion) = UPPER('fCaCjLiquidacion')
			BEGIN
				...
				EXEC pSDValidaLiquidacionAnticipada @Codigo, @MontoDeposito, @CodSistema, @CodOficina

				--GDF-20220629 Begin (Se agrega validación de recuperaciones montos mayores pesos)
				EXEC pSDValidaRecMontosMayoresPesos @Codigo, @MontoDeposito, @CodSistema, @CodOficina
				--GDF-20220629 End
			END
