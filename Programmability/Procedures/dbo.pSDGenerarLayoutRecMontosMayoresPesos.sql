...
BEGIN
	...
	SET @SQL = N'IF OBJECT_ID(N''#tmpLayoutRecMontosMayoresPesos'') IS NOT NULL
					BEGIN
						DROP TABLE #tmpLayoutRecMontosMayoresPesos 
					END
			   
			   CREATE TABLE #tmpLayoutRecMontosMayoresPesos (
					RowId INT IDENTITY(1,1),
					DataInfo NVARCHAR(2000)
			   )

			   INSERT INTO #tmpLayoutRecMontosMayoresPesos (DataInfo) 
			   SELECT ''2;'' + CONVERT(VARCHAR(10),GETDATE(),112) + '';??????;001002;027004;' + @CodigoLocalidad + ';'' + c.CodPostal + '';' + 
					@TipoOperacion + ';01;'' + REPLACE(a.Codigo,''-'','''') + 
			   ...
			   FROM tAlertasRecMontosMayoresPesosPLD a WITH (NOLOCK)
			   LEFT JOIN Finmas.dbo.tAhCuenta b WITH (NOLOCK) ON a.Codigo = b.CodCuenta
			   LEFT JOIN Finmas.dbo.tClOficinas c WITH (NOLOCK) ON a.CodOficina = c.CodOficina
			   LEFT JOIN Finmas.dbo.tUsUsuarios d WITH (NOLOCK) ON a.CodUsuario = d.CodUsuario
			   LEFT JOIN Finmas.dbo.tUsUsuarioDireccion e WITH (NOLOCK) ON a.CodUsuario = e.CodUsuario AND e.FamiliarNegocio = ''F''
	...
	SET @SQL = @SQL + 'SELECT REPLACE(DataInfo,''??????'',RIGHT(''000000'' + CAST(RowId AS VARCHAR(6)),6)) AS Linea FROM #tmpLayoutRecMontosMayoresPesos WITH (NOLOCK) 
					  DROP TABLE #tmpLayoutRecMontosMayoresPesos'
	--PRINT @SQL
