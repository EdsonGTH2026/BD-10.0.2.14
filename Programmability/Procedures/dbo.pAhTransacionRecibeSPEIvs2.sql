...
GO
CREATE PROCEDURE [dbo].[pAhTransacionRecibeSPEIvs2]
( @IdTransaccion           INT, 
...
--BEGIN
	...
	IF NOT EXISTS (
							SELECT  IdTransaccion FROM tAhTransaccionDiariaRecibeSPEI WITH(NOLOCK)
			...
			BEGIN
				...
				BEGIN
					...
					BEGIN					
						...
						(	IdTransaccion, FechaOperacion, InstitucionOrdenante, InstitucionBeneficiaria, ClaveRastreo, Monto, NombreOrdenante, tipoCuentaOrdenante, CuentaOrdenante, 
							RfcCurpOrdenante, NombreBeneficiario, TipoCuentaBeneficiario, CuentaBeneficiario, RfcCurpBeneficiario, ConceptoPago, ReferenciaNumerica, Empresa, FechaHoraOpracion)
						VALUES
						(	@IdTransaccion, @FechaOperacion, @InstitucionOrdenante, @InstitucionBeneficiaria, @ClaveRastreo, @Monto, @NombreOrdenante, @tipoCuentaOrdenante, 
							@CuentaOrdenante, @RfcCurpOrdenante, @NombreBeneficiario, @TipoCuentaBeneficiario, @CuentaBeneficiario, @RfcCurpBeneficiario, @ConceptoPago, @ReferenciaNumerica, 
							@Empresa, GETDATE());
						
						EXEC Finmas.dbo.pAhDepositoCuentaSPEI_vs2  --PRODUCCION  
						@Codcuenta, @Monto, '98CSM1803891',	@ClaveRastreo,@IdTransaccion;
						--- VALIDA SI EN REALIDAD SE CREO UN REGISTRO DE LA OPERACION -----    
						
						IF NOT EXISTS(
							SELECT 1 FROM tAhTransaccionDiariaRecibeSPEI with(nolock)
							WHERE CuentaBeneficiario = @CuentaBeneficiario AND idtransaccion = @IdTransaccion AND Monto = @Monto)
							BEGIN							
	...
	ELSE
	BEGIN
		--Se asume que si esta registrado el id es que la transaccion ya fue aceptada
		SET @Response = 200 -- 1
		SELECT @Response
	END
--END;
