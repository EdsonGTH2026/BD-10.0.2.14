...
GO
CREATE PROCEDURE [dbo].[pSDBuscarPersonaBloqueadaPLDApp](@Nombre VARCHAR(100), @ApellidoPaterno VARCHAR(50), @ApellidoMaterno VARCHAR(50), 
			...
			IF @Count = 1
				BEGIN
					...
					IF @RptaRegla IS NULL	-- COINCIDE POR NOMBRE PERO NO POR SOLICITUD Y OFICINA, O NO EXISTE LA ALERTA
						BEGIN
							INSERT INTO tAlertasPersonasBloqueadas (IdPersonaBloqueada, RptaRegla, 
																	CodSistema, CodSolicitud, CodOficina, FechaSistema, CodUsuario)
							VALUES (@Id, 1, @CodSistema, @CodSolicitud, @CodOficina, GETDATE(), @CodUsuario)
					...
					ELSE IF @RptaRegla = 3	-- COINCIDE POR NOMBRE, SOLICITUD Y OFICINA PERO LA ALERTA YA FUE RECHAZADA
						...
						END
				END
			ELSE IF @Count <= 0 --SE CONSERVA PARA VALIDAR QUE NO HAYA COINCIDENCIAS
				BEGIN
					SET @Msg = ''
					SET @Subject = ''
					...
					--	BEGIN
							--************* SE QUITA EL INGRESO DE PERSONA A LA LISTA Y LA GENERACIÓN DE ALERTA PREVIAMENTE AUTORIZADA *************
							--************* A SOLICITUD DE REYNA (PLD) *************

							/*SELECT @RFC = UsRFCBD, @FechaNacimiento = FechaNacimiento FROM Finmas.dbo.tUsUsuarios AS a WITH (NOLOCK)
							WHERE CodUsuario = @CodUsuario
							
							INSERT INTO tListaPersonasBloqueadas (IdArchivo, IdTipo, Tipo, FechaOficio, Nombre, 
																	ApellidoPaterno, ApellidoMaterno, RFC, FechaNacimiento, 
																	NombreArchivo, FechaSistema)
							VALUES (0, 0, '', GETDATE(), @Nombre, @ApellidoPaterno, @ApellidoMaterno, @RFC, @FechaNacimiento, 
									'Agregado por coincidencia', GETDATE())
						
							SET @Id = SCOPE_IDENTITY()
						
							INSERT INTO tAlertasPersonasBloqueadas (IdPersonaBloqueada, RptaRegla, DictamenObservacion,
																	CodSistema, CodSolicitud, CodOficina, FechaRespuesta, FechaSistema, CodUsuario)
							VALUES (@Id, 2, 'Autorizada', @CodSistema, @CodSolicitud, @CodOficina, GETDATE(), GETDATE(), @CodUsuario)*/
						--END
				...
				BEGIN
					...
					IF @Id <= 0
						BEGIN
							...
							INSERT INTO tAlertasPersonasBloqueadas (IdPersonaBloqueada, RptaRegla, DictamenObservacion,
																	CodSistema, CodSolicitud, CodOficina, FechaSistema, CodUsuario)
							VALUES (@Id, 1, 'Por revisar', @CodSistema, @CodSolicitud, @CodOficina, GETDATE(), @CodUsuario) 
						END
