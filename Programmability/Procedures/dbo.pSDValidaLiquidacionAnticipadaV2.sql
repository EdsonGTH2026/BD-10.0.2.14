...
GO
CREATE PROCEDURE [dbo].[pSDValidaLiquidacionAnticipadaV2](@Codigo VARCHAR(25), @MontoDeposito MONEY, @CodSistema VARCHAR(3), 
...
BEGIN
	...
	BEGIN
		SELECT @Cuotas = a.Cuotas, @CodTipoPlazo = a.CodTipoPlaz, @DiasTrans = DATEDIFF(DAY, a.FechaDesembolso, GETDATE()), 
			   @MontoDesembolso = a.MontoDesembolso, @CodUsuario = a.CodUsuario
		...
		IF @GenerarAlerta = 1
			BEGIN
				INSERT INTO tAlertasDepositosAnticipadosPLD (Codigo, Monto, CodUsuario, NombreCliente, RptaRegla, CodSistema, CodOficina, FechaSistema, SecuenciaPago)
				VALUES (@Codigo, @MontoDeposito, @CodUsuario, @NombreCliente, 1, @CodSistema, @CodOficina, GETDATE(), @SecPago)
