...
GO
create procedure [dbo].[pSICRegistraConsultaMonitor_CC](@CodUsuario varchar(20), @Cuenta varchar(20), @CodOficina varchar(3))
as
begin
...
--ver 06-05-2021: Se cambio la condicion para que se vuelva a insertar la consulta solo si la consulta no tiene Procesado in 0, 1 y 2 
	...
	if not exists(select * from tCcConsulta where CodUsuario = @CodUsuario and  Cuenta = @Cuenta and CodOficina= @CodOficina and Procesado in (0,1,2) and Activo = 1)
	begin 
		...
		values (@CodUsuario, getdate(), '', 0, null, '', '', null, null, 'M', @Cuenta, @CodOficina, null, null, '', 0, 1)
	end
end
